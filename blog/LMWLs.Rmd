...
* theoretical background of the GMWL  
* Data pre-processing   
* linear regression analyses (LMWLs)  
* result visualization in geographic map  
...
# Data pre-processing
To calculate the LMWLs of different stations of Germany by using the GNIP database some pre-processing is a must.
Fortunately the whole database is already read to a tidy data frame, as described within the  [tutorial](http://n-dim.de/tutorial/webscraping-with-rvest.html).
...
```{r, echo=TRUE, fig.height=8, fig.width=7.4, message=FALSE, warning=FALSE}
...
```{r, echo=T, fig.height=8, fig.width=7.4, message=FALSE, warning=FALSE}
